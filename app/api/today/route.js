...
async function assembleMLBGame(game, oddsMap) {
  try {
    ...
    return {
      ...
      openingAwayML: odds.openAway ? (odds.openAway > 0 ? `+${odds.openAway}` : `${odds.openAway}`) : odds.awayML || 'N/A',
      openingHomeML: odds.openHome ? (odds.openHome > 0 ? `+${odds.openHome}` : `${odds.openHome}`) : odds.homeML || 'N/A',
