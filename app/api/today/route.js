...
async function assembleMLBGame(g, oddsMap) {
  ...
  const homeRecord      = val(0, { overall:'N/A', home:'N/A', away:'N/A', last5:'N/A', last10:'N/A', streak:'N/A' });
  const awayRecord      = val(1, { overall:'N/A', home:'N/A', away:'N/A', last5:'N/A', last10:'N/A', streak:'N/A' });
  const homePitcherStats = val(2, 'Stats unavailable');
  const awayPitcherStats = val(3, 'Stats unavailable');
  const cbsPreview      = val(4, 'Preview unavailable');
  const mlbH2H          = val(5, 'H2H unavailable');
  const rotoWireInjuries = val(6, 'Check rotowire.com');
  const weather         = val(7, 'Weather unavailable');
  const umpire          = val(8, 'Umpire TBD');
  const homeLineupData  = val(9, { lineup: 'Not yet confirmed', splits: 'Unavailable' });
  const awayLineupData  = val(10, { lineup: 'Not yet confirmed', splits: 'Unavailable' });
  const homePitcherVsAway = val(11, 'Splits unavailable');
  const awayPitcherVsHome = val(12, 'Splits unavailable');
  return {
