...
8. MARKET SELECTION — you MUST evaluate the TOTAL and SPREAD/RUN LINE before choosing ML. ML should NOT appear on every game. If pitching matchup, weather, park factors, and offensive form point to a low-scoring game → pick the UNDER. If one team is dominant and expected to win by multiple runs/points → pick the spread/run line. ML is only correct when the edge is real but margin is genuinely uncertain. A slate where every pick is ML means the other markets were not properly evaluated.
