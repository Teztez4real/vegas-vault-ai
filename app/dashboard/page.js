...
export default function DashboardPage() {
  ...
  if (status === 'loading') {
    return (
      <div style={{ minHeight:'100vh', background:'#07091a', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'DM Mono',monospace" }}>
        <div style={{ fontSize:12, color:'#3a4a5e', letterSpacing:'0.1em' }}>LOADING...</div>
      </div>
    );
  }
    ...
    return (
      <div>
        <div style={{ position:'fixed', top:0, left:0, right:0, height:40, background:'rgba(201,162,39,0.95)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px', zIndex:9999, fontFamily:"'DM Mono',monospace" }}>
          ...
          <button onClick={() => setView('main')} style={{ fontSize:11, fontWeight:700, color:'#000', background:'rgba(0,0,0,0.15)', border:'1px solid rgba(0,0,0,0.2)', borderRadius:6, padding:'4px 12px', cursor:'pointer', fontFamily:'inherit' }}>← Back to Dashboard</button>
        </div>
        <div style={{ paddingTop:40 }}><AdminDashboard /></div>
      </div>
    );
  }
    ...
    <div>
      {adminUser && (
        <div style={{ position:'fixed', bottom:70, right:16, zIndex:9999 }}>
          <button onClick={() => setView('admin')} style={{ background:'linear-gradient(135deg,#c9a227,#8b6d10)', border:'none', borderRadius:10, padding:'10px 16px', fontSize:11, fontWeight:700, color:'#000', cursor:'pointer', letterSpacing:'0.08em', boxShadow:'0 4px 20px rgba(201,162,39,0.4)', fontFamily:"'DM Mono',monospace", display:'flex', alignItems:'center', gap:8 }}>
            ⚡ ADMIN PANEL
          </button>
        </div>
      )}
