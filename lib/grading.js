...
// ── DETECT + INVALIDATE WRONG-SPORT ANALYSIS CONTENT ──────────────────────────
// Fixes what a code change alone can't: analyses already generated and stored
// BEFORE the Stage-1 sport-branching fix (see generate/route.js) can still
// contain literal baseball fields ("Away starter", "bullpen", "batter splits
// vs pitcher") inside a non-MLB game's saved reasoning. Rather than requiring
// a manual Force Re-analyze on every affected game, this scans every
// not-yet-started game_analyses row for these telltale phrases and — ONLY
// for games that haven't started yet, never a completed/graded game's
// historical record — clears the stored result so the normal auto-analyze
// cron picks it up as "needs analysis" again and regenerates it fresh with
// the corrected, sport-aware code. Runs automatically on the cron; no manual
// step needed. Safe to run every cycle — once a game's analysis is clean, it
// won't match these phrases again and won't be touched.
const WRONG_SPORT_MARKERS = [
  'Away starter:', 'Away bullpen:', 'Home bullpen:',
  'batter splits vs pitcher', 'starter vs this opponent',
  'not a baseball game', // our own new N/A marker leaking would be equally wrong to display
];

export async function invalidateWrongSportAnalyses(sb, date) {
  try {
    const { data: rows } = await sb.from('game_analyses').select('game_key, sport, result').eq('date', date).neq('sport', 'MLB');
    if (!rows?.length) return 0;

    let invalidatedCount = 0;
    for (const row of rows) {
      if (!row.result) continue;
      const hasMarker = WRONG_SPORT_MARKERS.some(m => row.result.includes(m));
      if (!hasMarker) continue;

      // Never touch a game that's already graded (has a real win/loss result
      // locked in) — the GRADE itself isn't affected by this bug (that's a
      // separate, correct process), only the displayed reasoning text. Only
      // invalidate ungraded/upcoming analyses so users see correct reasoning
      // before they need to act on it.
      let parsed; try { parsed = JSON.parse(row.result); } catch { continue; }
      if (parsed?.graded) continue;

      await sb.from('game_analyses').delete().eq('game_key', row.game_key);
      invalidatedCount++;
    }
