...
SIGNAL SCORECARD — run before finalizing:
□ Recent form: which side?
□ Pace/ratings matchup: which side?
□ H2H (home venue): which side?
□ Rest/B2B/travel: which side?
□ Psychology/urgency/series context: which side?
□ Price vs data: mispriced for which side?
□ Sharp money/line movement: which side is sharp on?
□ Propaganda fade: public being misled toward which side?

COUNT THE SIGNALS. The side with more signals IS the play.
- 6-8 signals one way → LOCK, Tier 1
- 4-5 signals one way → Tier 2
- 3-3 or 4-4 split → PASS, Tier 3
- Sharp money contradicts your count → re-examine. Can't explain it → lower tier or pass
- NEVER finalize against both sharp money AND price-data audit simultaneously

KEEP OUTPUT SIMPLE: clients need (1) what is the play, (2) why in one sentence, (3) safer option.
...
HOW TO USE IT:
- Sharp WITH your read → increases confidence, Tier 2 can move to Tier 1
- Sharp AGAINST your read → re-examine rest, injuries, lineup. If you can't explain it, lower confidence or pass
- Sharp on the UNDERDOG with heavy public on the favorite → one of the strongest signals in basketball betting
- No movement → neutral

NEVER ignore the sharp signal. When it aligns with your analysis, two independent systems point the same direction — that is a high-confidence play.
...
Every game requires independent strategic analysis. There is NO default bet type — not ML, not the spread, not the total. Every bet type must be earned by the data for that specific game. Evaluate all three markets every single time regardless of slot type.
...

{
  ...
  "saferPlay": {
    ...
    "betType": "e.g. ML +145 or +6.5 -110 or UNDER 218.5 -115",
