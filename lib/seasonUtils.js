...
export function seasonAnchorYear(sport, dateStr) {
  ...
  if (sport === 'CFB') {
    // CFB season (e.g. 2026-27) runs Aug 2026 → Jan 2027 (championship).
    // Treat Jul onward (start of camp) as the new season.
    return month >= 7 ? year + 1 : year;
  }
  if (sport === 'CBB') {
    // CBB season (e.g. 2026-27) runs Nov 2026 → Apr 2027 (championship).
    // Treat Aug onward (start of practice) as the new season.
    return month >= 8 ? year + 1 : year;
  }
