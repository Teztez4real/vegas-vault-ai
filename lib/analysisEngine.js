...
Return JSON:
{
  ...
  "analysis": {
    ...
    "pitching": "Pitching matchup summary. CRITICAL: if the provided vs-opponent data only covers this season and shows thin/zero starts, you MUST use web search to check the starter's career history against this specific opponent (Baseball Reference, FanGraphs, ESPN) before writing this field — never state a pitcher 'has never faced' a team based on season-only data alone, since that's almost always false for any pitcher with multiple years of MLB service. If search genuinely turns up no prior meetings, only then say so.",
