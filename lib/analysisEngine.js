...
- Injuries: ${game.injuries || 'Check both rosters — note any IL placements, day-to-day players, or lineup absences'}
- Weather: ${game.weather || 'Check conditions — note temperature, wind speed/direction, precipitation'}
- Umpire: ${game.umpire || 'N/A — note if known strikeout or walk-heavy umpire'}
- Moneyline: Away ${game.awayML || 'N/A'} / Home ${game.homeML || 'N/A'}
- Run Line: Away ${game.spread ? (parseFloat(game.spread) > 0 ? '+' : '') + (-parseFloat(game.spread || 0)).toFixed(1) : 'N/A'} ${game.awaySpreadPrice || '-110'} / Home ${game.spread || 'N/A'} ${game.homeSpreadPrice || '-110'}
- Total: ${game.total ? 'o' + game.total : 'N/A'} ${game.overPrice || '-110'} / u${game.total || 'N/A'} ${game.underPrice || '-110'}
...
Return JSON:
{
  "awayFacts": "3-4 key facts about the away team right now — form, streak, recent run production",
  "homeFacts": "3-4 key facts about the home team right now — form, streak, recent run production",
  "recentForm": "MANDATORY: Away last 5 and last 10 results. Home last 5 and last 10 results. Who is hot, who is cold, and what the trend shows",
  "headToHead": "MANDATORY: Overall H2H last 5-10 matchups AND the last time these teams played at this specific home venue (go back to last season if needed). State result, margin, and whether one team owns this venue matchup",
  "pitchingFacts": "key pitching facts for both sides",
  "situationalFacts": "series context — just facts",
  "injuries": "MANDATORY: list any IL players, day-to-day players, or key absences. If none, state No significant injuries reported",
  "weather": "MANDATORY: temperature, wind speed and direction, precipitation and how it affects the game",
...
Weather: ${stage1Data.weather || 'N/A'}
...
MONEYLINE: Away ${game.awayML || 'N/A'} / Home ${game.homeML || 'N/A'}
${isBaseball ? `RUN LINE: Away ${game.spread ? (parseFloat(game.spread)>0?'+':'')+ (-parseFloat(game.spread||0)).toFixed(1) : 'N/A'} ${game.awaySpreadPrice||'-110'} / Home ${game.spread||'N/A'} ${game.homeSpreadPrice||'-110'}` : `SPREAD: ${game.spread || 'N/A'}`}
TOTAL: o${game.total||'N/A'} ${game.overPrice||'-110'} / u${game.total||'N/A'} ${game.underPrice||'-110'}
PITCHING: ${stage1Data.pitchingFacts}
SITUATION: ${stage1Data.situationalFacts}
...
export function buildNBAStage1Prompt(game) {
  return `You are a basketball data analyst. Summarize the factual state of both teams. No picks. No lean. Just facts.

GAME: ${game.away} @ ${game.home} | NBA | ${game.time}
SLOT: ${game.slot || 'PUBLIC'}
...
- Away Last 5: ${game.awayLast5 || 'N/A'} | Home Last 5: ${game.homeLast5 || 'N/A'}
- Away Last 10: ${game.awayLast10 || 'N/A'} | Home Last 10: ${game.homeLast10 || 'N/A'}
- Away ATS: ${game.awayATS || 'N/A'} | Home ATS: ${game.homeATS || 'N/A'}
- Away PPG: ${game.awayPPG || 'N/A'} | Home PPG: ${game.homePPG || 'N/A'}
- Away Opp PPG: ${game.awayOppPPG || 'N/A'} | Home Opp PPG: ${game.homeOppPPG || 'N/A'}
- Away Off Rating: ${game.awayOffRating || 'N/A'} | Home Off Rating: ${game.homeOffRating || 'N/A'}
- Away Def Rating: ${game.awayDefRating || 'N/A'} | Home Def Rating: ${game.homeDefRating || 'N/A'}
- Away Pace: ${game.awayPace || 'N/A'} | Home Pace: ${game.homePace || 'N/A'}
- H2H Last 5: ${game.h2hLast5 || 'N/A'}
- H2H At Home Venue: ${game.h2hAtHome || 'N/A'}
- Injuries: ${game.injuries || 'None reported'}
- Rest: Away ${game.awayRest || 'N/A'} days | Home ${game.homeRest || 'N/A'} days
- B2B: Away ${game.awayB2B ? 'YES' : 'No'} | Home ${game.homeB2B ? 'YES' : 'No'}
- Moneyline: Away ${game.awayML || 'N/A'} / Home ${game.homeML || 'N/A'}
- Spread: ${game.spread || 'N/A'} (Away ${game.awaySpreadPrice||'-110'} / Home ${game.homeSpreadPrice||'-110'})
- Total: o${game.total||'N/A'} ${game.overPrice||'-110'} / u${game.total||'N/A'} ${game.underPrice||'-110'}
- Line Movement: ${game.lineMovement || 'N/A'}
- Sharp Signal: ${game.sharpSignal || 'N/A'}
- Series/Playoff Context: ${game.seriesContext || game.playoffContext || 'N/A'}

Return JSON:
{
  "awayFacts": "3-4 key facts about the away team right now — form, ratings, key players",
  "homeFacts": "3-4 key facts about the home team right now — form, ratings, key players",
  "recentForm": "MANDATORY: Away last 5 and last 10 results. Home last 5 and last 10 results. Who is hot, who is cold, trending up or down",
  "headToHead": "MANDATORY: Overall H2H last 5-10 matchups AND the last time these teams played at this specific home arena (go back to last season if needed). Result, margin, and whether one team owns this venue",
  "matchupFacts": "pace matchup, offensive vs defensive rating matchup, stylistic edge",
  "situationalFacts": "rest, B2B, injuries, playoff context, series situation",
  "lineFacts": "current prices, line movement, sharp signal — just facts"
}`;
}
...
${isVegas ? `VEGAS SLOT SCAM: Look harder — what is the public betting and is the data supporting it?
- Is a big-market team (Lakers, Celtics, Warriors) overpriced due to name brand?
- Is a team on a hot streak getting overvalued by the public?
- Is the real edge on the side everyone is ignoring?` : `PUBLIC SLOT: Go with the better team unless data actively contradicts them.`}
...
SHARP MONEY: One signal. If sharp money aligns with your read → adds confidence. If it contradicts → note it, don't flip automatically.
...
MONEYLINE: Best when edge is clear on winner but margin is uncertain. Good for competitive matchups where you're confident on side but not by how much.
...
Return JSON:
{
  ...
  "analysis": {
    ...
    "sharpMoney": "Sharp signal and how much weight it carries",
    "propaganda": "Public narrative vs reality",
    "scamPlay": "${isVegas ? 'Why public is wrong and what reality is' : 'N/A'}",
...
export function buildNFLStage1Prompt(game) {
  return `You are a football data analyst. Summarize the factual state of both teams. No picks. No lean. Just facts.

GAME: ${game.away} @ ${game.home} | NFL | ${game.time}
SLOT: ${game.slot || 'PUBLIC'}
...
- H2H Last 5: ${game.h2hLast5 || 'N/A'} | H2H At Home: ${game.h2hAtHome || 'N/A'}
- Injuries: ${game.injuries || 'None reported'}
...
- Line Movement: ${game.lineMovement || 'N/A'}
- Sharp Signal: ${game.sharpSignal || 'N/A'}
- Week/Game Type: ${game.week || 'N/A'} | ${game.gameType || 'Regular Season'}

Return JSON:
{
  "awayFacts": "3-4 key facts — QB form, offensive/defensive strengths, current injuries",
  "homeFacts": "3-4 key facts — QB form, offensive/defensive strengths, current injuries",
  "recentForm": "MANDATORY: Away last 5 results and trend. Home last 5 results and trend. Who is playing well right now and who is struggling",
  "headToHead": "MANDATORY: Overall H2H last 5-10 matchups AND the last time these teams played at this specific home stadium (go back to last season if needed). Result, margin, and whether one team dominates this venue matchup",
  "matchupFacts": "key schematic matchup — how does each team's strength match up against the other's weakness?",
  "situationalFacts": "weather, week context, divisional game, rest, travel, injuries",
  "lineFacts": "current prices, line movement, sharp signal — just facts"
}`;
}
...
${isVegas ? `VEGAS SLOT SCAM: What is the public narrative and is it supported by data?
- Is a marquee team (Cowboys, Patriots brand) overpriced on name recognition?
- Is a team on a hot streak getting more credit than the matchup supports?` : `PUBLIC SLOT: Go with the better team. High bar to fade.`}
...
MONEYLINE: Best for underdogs or close games. NFL underdogs win outright ~44% of the time. Taking a +150 dog that wins 40% of the time is profitable. ML on heavy favorites is usually better expressed as the spread.
