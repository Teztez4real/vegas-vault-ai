...
ROSTER-FILL — WHERE THE FREED PRODUCTION GOES ("Robin to the Batman"): When a team's top scorer(s) are OUT, the freed-up usage, shots, and rebounds do not vanish — they redistribute. The public and the line inflate the OBVIOUS next-biggest name (usually overpriced), while the real, underpriced beneficiary is the non-obvious one who quietly absorbs the vacated role — the backup big living under the rim for rebounds, the secondary guard inheriting the ball. For the game itself, this changes who the offense actually runs through and therefore the realistic total and side. STRONGEST version: a meaningful line/total move with NO lineup change at all — that is pure line engineering, not a roster reaction, and it is the highest-value discrepancy.

RECENCY-BIAS LINE INFLATION: A line built off one loud recent result — a 25-30 point blowout win, a superstar's huge night — is a classic trap. The public extrapolates the last game; the number gets shaded toward the team that just looked dominant. Check WHY the recent result happened (opponent quality, who was resting, garbage time) and whether that reason still applies tonight. When a spread or total looks a full point or more off what the season-long picture supports because of one recent game, lean against the recency-inflated side.
