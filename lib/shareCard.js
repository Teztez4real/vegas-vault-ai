 ...
 *   hasSlotSystem: true|false,  // false for Tennis/WNBA — no Public/Vegas slots
 * }
 */
export async function generateShareCardBlob(data) {
  ...
  ctx.fill();
  // Once graded, tint the whole card's border to match the outcome (not
  // just the small chip inside) — makes the result unmistakable at a glance
  // rather than something you have to read for.
  const resultIsLoss = data.resultStamp && data.resultStamp.includes('LOSS');
  ctx.strokeStyle = data.resultStamp
    ? (resultIsLoss ? 'rgba(255,60,60,0.45)' : 'rgba(57,255,20,0.5)')
    : 'rgba(57,255,20,0.25)';
  ctx.lineWidth = data.resultStamp ? 3 : 2;
  roundRect(ctx, cardX, cardTop, cardW, cardH, 28);
  ctx.stroke();
  ...
  ctx.fillStyle = GREEN;
  ctx.font = '800 20px Arial, sans-serif';
  // Tennis and WNBA have no Public/Vegas slot system at all — label those
  // as a plain AI PICK rather than incorrectly calling them a "Public slot"
  // pick, which would misrepresent how the game was actually analyzed.
  const slotLabel = data.hasSlotSystem === false
    ? 'AI PICK'
    : (data.isVegasSlot ? 'VEGAS SLOT PICK' : 'PUBLIC SLOT PICK');
  ctx.fillText(slotLabel, cardX + 40, cy);
  ...
  // Tier + confidence chips (+ result stamp directly next to the play, when graded)
  cy += 20;
  ...
  let chipX = cardX + 40 + tierW + 20;

  if (data.resultStamp) {
    const stampIsLoss = data.resultStamp.includes('LOSS');
    ctx.font = '800 24px Arial, sans-serif';
    const stampW = ctx.measureText(data.resultStamp).width + 44;
    ctx.fillStyle = stampIsLoss ? 'rgba(255,60,60,0.14)' : 'rgba(57,255,20,0.14)';
    roundRect(ctx, chipX, cy - 30, stampW, 46, 12);
    ctx.fill();
    ctx.strokeStyle = stampIsLoss ? 'rgba(255,60,60,0.4)' : 'rgba(57,255,20,0.4)';
    ctx.lineWidth = 2;
    roundRect(ctx, chipX, cy - 30, stampW, 46, 12);
    ctx.stroke();
    ctx.fillStyle = stampIsLoss ? '#ff6b6b' : GREEN;
    ctx.fillText(data.resultStamp, chipX + 22, cy + 2);
    chipX += stampW + 20;
  }

  if (data.confidencePercent != null && !data.resultStamp) {
