...
- Spread: ${gameData.spread || 'N/A'}
- Total (O/U): ${gameData.total || 'N/A'}
- Line Movement: ${gameData.lineMovement || 'N/A'}
- Away Record: ${gameData.awayRecord} | Home Record: ${gameData.homeRecord}
- Away Last 5: ${gameData.awayLast5 || 'N/A'} | Home Last 5: ${gameData.homeLast5 || 'N/A'}
- Away Last 10: ${gameData.awayLast10 || 'N/A'} | Home Last 10: ${gameData.homeLast10 || 'N/A'}
- Injuries: ${gameData.injuries || 'Check rotowire.com'}
- Weather: ${gameData.weather || 'Indoor arena — N/A'}
- H2H: ${gameData.h2h || 'N/A'}
- Recent Form Details: ${gameData.recentForm || 'N/A'}
- Public Betting %: ${gameData.betPercentage || 'N/A'}
- Sharp Money %: ${gameData.moneyPercentage || 'N/A'}

WNBA-SPECIFIC CONTEXT:
- WNBA lines are less sharp than NBA — market inefficiencies are more common and more exploitable
- Fatigue and back-to-backs hit harder in WNBA due to compressed schedule and less recovery infrastructure
...
- Home court is significant — smaller arenas, louder crowds, real impact
- Public overreacts to last game results — fade extremes
- Pace and total analysis: slow-pace teams lean UNDER, fast-pace teams inflate public OVER expectations
- Officiating tendencies affect foul rates and pace heavily in WNBA
...
STEP 1 — MATCHUP FOUNDATION
Ignore the line completely. Who should win this game based purely on the matchup? Roster quality, coaching, system, momentum. This is the truth layer.

STEP 2 — RECORDS
Analyze overall record, home record, away record. Include winning streaks. Both teams.

STEP 3 — RECENT FORM
Last 5 games = accuracy (who is playing well right now). Last 10 games = trend (consistency). Look for blowout wins vs close wins, real form vs fake form, back-to-back fatigue signs.

STEP 4 — HEAD-TO-HEAD (H2H)
Last 5-10 matchups, who controls the series, margin of victory. If the home team is playing at home, look specifically at the last time these teams played at this venue.

STEP 5 — LINEUP / ROSTER ANALYSIS
Evaluate both rosters: depth, star player matchups, backcourt vs frontcourt battles, pace preferences, scoring systems. Who has the personnel advantage tonight?

STEP 6 — FATIGUE, TRAVEL & SCHEDULE
Back-to-back? Cross-country travel? Days of rest? WNBA schedule is compressed — fatigue edges are real and underpriced by the market. Identify which team has the rest/travel advantage.

STEP 7 — GAME SCRIPT
Classify as: Close game (1-5 point diff → favors ML or spread +), Blowout potential (big mismatch → favors spread -), or Controlled game (one team leads but not dominant → favors ML). Also project: high-scoring or low-scoring game? Pace matchup for the total.

STEP 8 — SERIES / SCHEDULE CONTEXT
Where are these teams in the season? Playoff positioning pressure? Revenge game? Team coming off embarrassing loss? Teams fighting for playoff spots play differently. Identify any situational edge.

STEP 9 — TRELL RULE (INJURY SYSTEM)
ACTIVATES ONLY on: First game a star player is OUT → bet ON that team (market overcorrects). First game a star player RETURNS → bet AGAINST that team (public overvalues return).
Does NOT apply if player has been out multiple games. Check injuries data. State: ACTIVE or INACTIVE.

STEP 10 — PRICING COMPREHENSION
Does the line make sense given the matchup? Should this team be favored? Should it be favored this much? Is there a pricing mismatch? Paying juice is acceptable if it matches the analysis.

STEP 11 — LINE MOVEMENT
Confirmation only. Movement toward your side = confidence increases. Movement against you = re-check everything. Sharp money in WNBA is rare but powerful when it appears.

STEP 12 — PUBLIC vs SHARP (VEGAS vs PUBLIC SYSTEM)
No slot pattern in WNBA — use pure sharp psychology:
- Where is the public money going? (follow star power, popular teams, last game results)
- Where is sharp money going? (reverse line movement, book limits, steam moves)
- Is public >60% on one side while line moves the other way? That is the signal.
- Identify: is this a PUBLIC play (go with public narrative) or a SHARP play (fade the public)?

STEP 13 — SCAM PLAY IDENTIFICATION (MANDATORY EVERY GAME)
Label: SCAM PLAY or STANDARD PLAY
WHY IT LOOKS WRONG: What is the public narrative? Why does the fade look bad on the surface?
WHY IT'S ACTUALLY CORRECT: The real matchup breakdown, the pricing mismatch, the series context, the sharp signal.

STEP 14 — DISCREPANCY ANALYSIS (ALL 4, EVERY GAME)
1. LINE DISCREPANCY: Book gap of 5+ points across DraftKings, FanDuel, BetMGM, Caesars, Bet365? Flag it.
2. PUBLIC vs SHARP: >60% public on one side + line moving opposite = reverse line movement edge.
3. OPENING vs CURRENT: How many points has the line moved? Direction? 5+ points = significant. 10+ = major sharp signal.
4. MODEL vs MARKET: Does your matchup conclusion match the price or contradict it? The gap is the edge.

STEP 15 — TIER, BET TYPE & FINAL VERDICT
Tier 1 (LOCK): Matchup clearly favors side, game script aligns, no contradictions, sharp confirmation.
Tier 2: Good edge, some uncertainty.
Tier 3: Weak edge or too confused — AUTOMATIC PASS.

BET TYPE RULES:
- ML: Clear team edge, fair price
- Spread (−): Dominance expected, blowout game script
- Spread (+): Competitive underdog, value at the number
- OVER: Fast pace, weak defenses, hot offensive teams, public fade opportunity
- UNDER: Elite defense matchup, slow pace, fatigue, cold shooting trends
NEVER default to ML — always evaluate spread and total.

FINALIZATION RULES — readyToFinalize defaults to FALSE. Only set true when ALL of these are met:
- Tier 1 LOCK with HIGH confidence AND odds are confirmed AND game is within 3 hours of start
- Tier 2 with HIGH confidence AND game is within 2 hours AND all data confirmed
- Tier 3 or PASS: ALWAYS false
- When in doubt: set false. The system re-analyzes throughout the day.

SPEED RULE: Each step 1-2 sentences max. No long paragraphs. State the finding and move on.

Respond in this EXACT JSON format with NO extra text:
{
  "summary": {
    "matchupFoundation": "Step 1: who wins and why",
    "records": "Step 2: record analysis both teams",
    "recentForm": "Step 3: last 5 and last 10 analysis",
    "headToHead": "Step 4: H2H history and control",
    "lineupAnalysis": "Step 5: roster and personnel edge",
    "fatigueTravel": "Step 6: rest, travel, back-to-back edge",
    "gameScript": "Step 7: Close / Blowout / Controlled + pace/total lean",
    "seriesContext": "Step 8: season/schedule situational edge",
    "trellRule": "Step 9: ACTIVE or INACTIVE — if active state player, team, direction",
    "pricingComprehension": "Step 10: is the line priced correctly?",
    "lineMovement": "Step 11: sharp money direction",
    "vegasVsPublicPropaganda": "Step 12: public vs sharp — PUBLIC play or SHARP play and why",
    "scamPlay": {
      "active": true,
      "whyItLooksWrong": "Step 13: why public fades this side",
      "whyItsActuallyCorrect": "Step 13: the real edge"
    },
    "discrepancies": "Step 14: all 4 discrepancies — book gap, public/sharp, opening/current, model/market",
    "tier": "Tier 1",
    "tierLabel": "LOCK",
    "pick": "Team name",
