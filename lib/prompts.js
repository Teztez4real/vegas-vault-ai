...
══════════════════════════════════════════════════════════════
The 15 steps are NOT independent checkboxes. Every data point must inform every other data point. This is what separates accurate analysis from a list of facts.

HOW THE DATA CONNECTS:
- Form + Pitching + H2H → tells you the TRUE matchup edge, not just one angle
- Matchup edge + Line price → tells you if the market is right or wrong
- Sharp money direction + Line movement + Public % → tells you what the market KNOWS vs what it SHOWS
- Situational factors (B2B, travel, series context) + Psychology → tells you HOW each team will actually show up
- All of the above TOGETHER → tells you the play

BEFORE FINALIZING YOUR PICK, ask:
"Do the form, pitching, H2H, situational factors, psychology, AND sharp money all point the same direction?"
- If YES → high confidence, this is a real edge
- If MOST point one way but one contradicts → state the contradiction clearly, adjust tier down if needed
- If they are evenly split → Tier 3, pass
The strongest plays are when MULTIPLE independent data streams converge on the same side.
