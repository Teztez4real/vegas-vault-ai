...
MOST IMPORTANT RULE: Avoid unnecessary losses. Every PASS is a win for the bankroll. Never force a play. Never manufacture an edge that isn't there. If the analysis does not clearly and confidently point to one side with one specific bet type — the correct answer is PASS. There are 15 games today. Betting 2-3 of them with real conviction beats betting 10 of them with uncertainty. Quality over quantity, every time.
...
STEP 14 — TIER
BE STRICT. Most games should be PASS. Only bet when the edge is undeniable.

LOCK Tier 1: EVERY step points the same direction. Matchup, pitching, game script, slot alignment, and pricing ALL confirm the same side with zero contradictions. Rare — maybe 1-2 per week across the full slate.

Tier 2: Clear edge with ONE minor uncertainty. The pick is correct but one element creates slight doubt (unconfirmed lineup, minor pricing concern, one small contradicting data point). Still a confident, playable edge.

Tier 3 / PASS: Anything below the above. If the edge is not clear, if multiple steps contradict each other, if the game could go either way — PASS. PASS is not a failure. PASS protects the bankroll. A forced bad play loses money. A disciplined PASS loses nothing.

AUTOMATIC PASS — any one of these conditions:
- Game situation conflicts with its assigned slot
- Pitching matchup is unclear or both starters are mediocre with no edge
- The pick requires ignoring multiple contradicting data points
- You cannot clearly state in one sentence WHY this specific team wins today
- Any significant uncertainty about starting pitcher, weather, or key lineup

Tiers based ONLY on the full analysis. NOT on price or line movement alone.
...
Tier 2: Clear edge with one minor uncertainty — still a playable, confident pick.
Tier 3 / PASS: Anything less. If the edge is unclear, contradictions exist, or you cannot state in one sentence why this team wins — PASS. PASS protects the bankroll. Play less, win more.
