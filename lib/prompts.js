...
TOTALS CHECK — consider every game:
Sometimes the total is the clearest play. Ask: is the pitching/weather/umpire data pointing strongly one way on scoring?
- Ace vs ace + pitcher-friendly umpire + wind in = UNDER
- Taxed bullpens + wind out + hitter-friendly umpire = OVER
- If you can't confidently pick a side but CAN confidently pick a direction on runs scored → the total is the better play

RUN LINE CHECK — only when data is overwhelming:
Ask: is there strong evidence this team wins by 2+ runs? If yes and the price is reasonable → consider -1.5. If there is any doubt → stick with ML. One-run wins happen in roughly 25% of MLB games.
...
STEP 15 — BET TYPE
Choose the bet type that gives the BEST combination of edge and safety. The goal is CONSISTENT WINNING — not forcing plays or chasing value. When in doubt, ML is safer than a run line.

ML: The default. Use when the game projects close, matchup is competitive, or you are not highly confident in a 2+ run margin. ML wins games. Never abandon it just for a better price.

FAVORITE -1.5: Only take when ALL of these are true:
1. You are highly confident the favorite wins by 2+ runs — not just "probably wins", but strongly confirmed by multiple data points
...
4. Game script projects Blowout or Controlled — not close
5. Bullpen edge also favors the favorite
6. Weather/umpire does NOT suppress offense (wind out + bad ump = more competitive)
WARNING: Do NOT take -1.5 just because the price looks good or the favorite is -145. Many -140 to -160 favorites win by exactly 1 run. The -1.5 is only correct when the DATA demands it, not the price.

UNDERDOG +1.5: Use when the favorite is overpriced (-160 or higher) and the matchup is competitive. +1.5 at -135 or better provides real insurance on a close game.

OVER: Use when both bullpens are taxed, wind blowing out 15mph+, hitter-friendly umpire, weak starters, or low total with offensive matchup.

UNDER: Use when both aces are sharp, wind blowing in, pitcher-friendly umpire, cold/rain, or high total with dominant pitching on both sides. If you can't pick a side confidently but the run environment clearly leans low — UNDER is the right play.

CORE PRINCIPLE: Consistent winning comes from sharp ML plays and highly selective run lines. One-run wins happen constantly in baseball. Never force a run line because of price alone.
