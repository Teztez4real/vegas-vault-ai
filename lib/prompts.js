...
Classify the game and determine the CORRECT bet type — not just the winner:

BLOWOUT (5+ run margin expected): Ace vs weak starter, dominant lineup edge, bullpen mismatch → -1.5 run line is the play. Do NOT take ML when -1.5 is available at +100 or better.
CONTROLLED (3-4 run margin): One team clearly better but not a blowout → -1.5 if odds are +100 or better, otherwise ML.
CLOSE (1-2 run game): Evenly matched → ML favorite or +1.5 underdog.
UNDERDOG SPOT: Dog has real win equity but ML is overpriced → +1.5 at reasonable juice.

RUN LINE RULE — apply every single game:
If the favorite ML is -140 or higher AND -1.5 is priced at +100 or better → the run line is almost always the correct play. This is the single most underused edge in baseball betting. The public habitually takes ML. Sharp money takes the run line when the price is right.
Example: Team is -160 ML / -1.5 at +110 → Taking -1.5 at +110 means you win MORE money if they win by 2+, and you only lose the bet if they win by exactly 1. That is almost always better value than -160 ML.

TOTALS RULE — consider every single game:
Sometimes the total is clearer than any side play. Ask: is the pitching/weather/umpire data screaming one direction on the total?
- Ace vs ace + pitcher-friendly umpire + wind blowing in = UNDER
- Taxed bullpens + wind blowing out + hitter-friendly umpire = OVER
- If you can't confidently pick a side but CAN confidently pick a direction on runs scored → bet the total instead
...
STEP 15 — BET TYPE
This is one of the most important decisions. Do NOT default to ML. Evaluate each option:

**FAVORITE -1.5 (Run Line):** Use when —
- Pitching matchup is dominant (ace vs weak starter, 1.5+ ERA difference)
- Bullpen advantage is significant
- Offense vs defense matchup heavily favors one side
- Game script projects as Blowout or Controlled (not a coin flip)
- -1.5 odds are +100 or better (free insurance on a win)
- Team has covered -1.5 in 60%+ of recent comparable spots
- Paying -130 ML or worse? The -1.5 at +105 or better is almost always the correct play

**UNDERDOG +1.5:** Use when —
- Underdog has legitimate win equity but ML price is too steep (over -160 on favorite)
- Pitching matchup is close
- Game script projects as competitive (1-3 run game)
- +1.5 at -130 or better = strong value play
- This is often safer than ML on underdogs priced +140 or higher

**ML:** Use when —
- Game is a true coin flip (both teams have similar win probability)
- Both -1.5 and +1.5 odds are unattractive
- Upset potential on underdog where you need the outright win
- Favorite is -120 or less (small enough juice that -1.5 isn't worth it)

**OVER:** Use when —
- Both offenses are hot, both bullpens are taxed or unreliable
- Wind blowing out 15mph+ at an outdoor park (Wrigley, Coors, etc.)
- Hitter-friendly umpire behind the plate
- Both starting pitchers have high ERA/WHIP or are on short rest
- Total is set low (7 or under) but matchup suggests a high-scoring game
- Both teams have hit the over in 60%+ of recent games

**UNDER:** Use when —
- Both starting pitchers are aces or in dominant recent form
- Wind blowing in 15mph+ at an outdoor park
- Pitcher-friendly umpire (low over/under historical rate)
- Both bullpens are fresh and reliable
- Cold weather or rain expected — suppresses offense
- Total is set high (9.5+) but both pitching staffs are strong
- Dome stadium with elite pitching matchup
- Both teams have hit the under in 60%+ of recent games

CRITICAL RULE: If neither side has a clear edge on the moneyline or run line, but the pitching/weather/umpire data strongly points one way — the TOTAL is often the best play. Never force a side play when the total is the clearest edge.

CRITICAL RULE: If the favorite is -140 or higher AND the run line is +100 or better, the run line (-1.5) is almost always the superior play. You are getting a discount on a team that should win by multiple runs.

Always choose the BEST play — ML, run line, or total. The run line and totals are frequently the sharpest plays in baseball and are both underused by the public.
