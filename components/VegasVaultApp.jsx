...
export default function VegasVaultApp() {
  ...
  // ── SHARED RESULTS POLL — every 5 min ────────────────────────────────────────
  // Merges server auto-updated analyses into local results so users see updated
  // picks automatically when the cron detects lineup/injury/line changes.
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Merge outside of setter to avoid setter-inside-setter
        setResults(prev => {
          const next = { ...prev };
          let changed = false;
          for (const [key, shared] of Object.entries(analyses)) {
            ...
            if (!prev[key] || sharedTime > userTime) {
        ...
        if (!cancelled) setSharedResults(analyses);
      } catch {}
    };
    poll();
