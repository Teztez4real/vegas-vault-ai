...
${gameData.awayPitcher} ${new Date().getFullYear()} season: ${gameData.awayPitcherStats}
${gameData.homePitcher} vs ${gameData.away} (career): ${gameData.homePitcherVsOpponent || 'No career data vs this opponent'}
${gameData.homePitcher} ${new Date().getFullYear()} season: ${gameData.homePitcherStats}
...
// ── AGENTS PANEL — real, live status of every automated worker ────────────────
// Fetches /api/agents/status (heartbeats written by the crons) so each agent's
// dot reflects what actually ran, not a hardcoded "Running". Self-contained so
// it can own its fetch/poll without touching the big component's state.
const AGENT_STATUS_LABEL = {
  on: 'Running', idle: 'Idle — nothing to do', stale: 'Stale — no recent run',
  error: 'Error on last run', unknown: 'Awaiting first run',
};
function agoLabelClient(iso) {
  if (!iso) return 'never';
...
function AgentsPanel() {
  ...
  useEffect(() => {
    let alive = true;
    const load = () => fetch('/api/agents/status')
      .then(r => r.json())
      .then(d => { if (alive) { setAgents(d.agents || []); setErr(false); } })
  ...
  const healthy = agents ? agents.filter(a => a.status === 'on' || a.status === 'idle').length : 0;

  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12, flex:1, minHeight:0, overflowY:'auto' }}>
      <div className="vv-glass" style={{ padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
        <div>
          <div style={{ fontSize:18, fontWeight:800, color:'#111', letterSpacing:-0.3 }}>Agents</div>
          <div style={{ fontSize:11, color:'#aaa', marginTop:2 }}>Your automated staff — each one runs on its own schedule, no app needed</div>
        </div>
        {agents && (
          <div style={{ fontSize:11, fontWeight:700, color: healthy===agents.length ? '#33aa00' : '#bb8800' }}>
            {healthy}/{agents.length} agents healthy
          </div>
        )}
      </div>

      {!agents && !err && (
        <div style={{ textAlign:'center', padding:'40px 0', color:'#aaa', fontSize:12 }}>Loading agent status…</div>
      )}
      {err && (
        <div className="vv-glass" style={{ padding:'16px 20px', fontSize:12, color:'#dd4444' }}>Couldn't load agent status right now — retrying every minute.</div>
      )}

      {agents && (
        <div className="vv-agent-grid">
          {agents.map(a => (
            <div key={a.key} className="vv-glass vv-agent-card">
              ...
              <div className={`vv-agent-status ${a.status}`}><div className={`vv-agent-std ${a.status}`}></div>{AGENT_STATUS_LABEL[a.status] || a.status}</div>
              <div className="vv-agent-desc">{a.desc}</div>
              <div className="vv-agent-mets">
                <div className="vv-am"><div className="vv-am-v">{a.lastRunAt ? agoLabelClient(a.lastRunAt) : '—'}</div><div className="vv-am-l">Last Run</div></div>
                <div className="vv-am"><div className="vv-am-v">{a.count != null ? a.count : '—'}</div><div className="vv-am-l">Processed</div></div>
              </div>
              {a.detail && <div style={{ fontSize:9, color:'#aaa', marginTop:6, lineHeight:1.4 }}>{a.detail}</div>}
            </div>
          ))}
        </div>
      )}
...
// ── DATA RECOVERY CARD ────────────────────────────────────────────────────────
function DataRecoveryCard({ authUser, setResults, setFinalized, setWatchlist, setPickHistory, setAltPicks }) {
  const [recovering, setRecovering] = useState(false);
  const [msg, setMsg] = useState('');

  const run = async () => {
    setRecovering(true);
    setMsg('Pulling data from Supabase…');
    try {
      const uid = authUser?.id;
      if (!uid) { setMsg('Not signed in.'); setRecovering(false); return; }
      ...
      setMsg('Merging with local cache…');
      ...
      setMsg(`✅ Restored — ${Object.keys(mergedRes).length} analyses, ${mergedHist.length} history entries, ${mergedWl.length} watchlist items`);
    } catch(e) {
  ...
  return (
    <div className="vv-glass vv-pad" style={{ border:'1px solid rgba(57,255,20,0.2)' }}>
      ...
      {msg && (
        <div style={{ fontSize:11, color: msg.startsWith('✅') ? '#33aa00' : msg.startsWith('❌') ? '#dd4444' : '#888', marginBottom:10, padding:'8px 12px', background:'rgba(0,0,0,0.03)', borderRadius:8, lineHeight:1.5 }}>
          {msg}
        </div>
      )}
      <button onClick={run} disabled={recovering} style={{ width:'100%', padding:'11px', borderRadius:10, border:'1px solid rgba(57,255,20,0.35)', background: recovering ? 'rgba(0,0,0,0.04)' : 'rgba(57,255,20,0.08)', color: recovering ? '#aaa' : '#33aa00', fontSize:12, fontWeight:800, cursor: recovering ? 'not-allowed' : 'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
...
export default function VegasVaultApp() {
  ...
  // Idle lock — show a lock screen but keep the Supabase session alive.
  ...
  // Picks up server analyses from the 3 daily crons and applies to all devices.
  // Only prev is read inside the setResults updater — no external state refs.
  useEffect(() => {
    if (!authUser?.id) return;
    let live = true;
    const poll = async () => {
      try {
        ...
        if (!data?.analyses || !live) return;
        const incoming = data.analyses;
        setResults(prev => {
          const out = { ...prev };
          let changed = false;
          Object.keys(incoming).forEach(key => {
            const st = new Date(incoming[key]?.updatedAt || 0).getTime();
            const lt = new Date(prev[key]?.updatedAt    || 0).getTime();
            if (st > lt) { out[key] = { ...incoming[key], _autoUpdated: true }; changed = true; }
          });
  ...
  useEffect(()=>{
    ...
    fetch(`/api/today?date=${selectedDate}`).then(r=>r.json())
      ...
      .catch((err)=>{setGames([]);setError(err.message||'Failed to load games');setLoading(false);});
  },[selectedDate]);
  ...
  const shellNavigate = (key) => {
    ...
    if (["dashboard","slate","vault","analytics","agents","memory","models","aichat"].includes(key)) {
      setShellView(key);
    }
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        .vv-agent-status.stale{color:#dd4444}
        .vv-agent-status.error{color:#dd4444}
        .vv-agent-status.unknown{color:#999}
        ...
        .vv-agent-std.stale{background:#dd4444;box-shadow:0 0 4px #dd4444}
        .vv-agent-std.error{background:#dd4444;box-shadow:0 0 5px #dd4444;animation:pulse 1.2s infinite}
        .vv-agent-std.unknown{background:#ccc}
        ...
        .vv-chat-ic{width:32px;height:32px;border-radius:9px;background:rgba(57,255,20,0.1);border:1px solid rgba(57,255,20,0.25);display:flex;align-items:center;justify-content:center}
        ...
        .vv-model-pill{font-size:9px;font-weight:700;color:#33aa00;border:1px solid rgba(57,255,20,0.3);padding:4px 11px;border-radius:9px;background:rgba(57,255,20,0.06);display:flex;align-items:center;gap:5px}
        ...
        .vv-msg.vv-user{align-self:flex-end;flex-direction:row-reverse}
        ...
        .vv-msg-av.vv-user{background:linear-gradient(135deg,#bbb,#999)}
        .vv-msg-av.vv-user i{color:#fff;font-size:15px}
        ...
        .vv-msg.vv-user .vv-msg-bubble{background:linear-gradient(135deg,#39FF14,#2ecc00);color:#0a2200;border-top-right-radius:4px;font-weight:500}
        ...
        .vv-msg.vv-user .vv-msg-time{text-align:right;color:rgba(10,40,0,0.4)}
        ...
        .vv-chat-input i.vv-attach{font-size:16px;color:#bbb;cursor:pointer}
        ...
        @media (max-width:900px){
          ...
          .vv-center{flex:none !important;grid-template-columns:1fr;height:auto;overflow-y:visible}
          ...
          .vv-slate{flex:none !important;width:100%;height:auto;overflow-y:visible;max-height:none}
      ...
      {authUser && isSubscribed && shellView === 'slate' && (
        <div style={{ display:'flex', flexDirection:'column', gap:12, flex:1, minHeight:0, overflowY:'auto' }}>
          ...
          {error && (
            <div style={{ padding:'12px 16px', background:'rgba(255,80,80,0.08)', border:'1px solid rgba(255,80,80,0.25)', borderRadius:10, fontSize:11, fontWeight:700, color:'#dd4444' }}>
              ⚠️ Couldn't load today's games: {error}. Try refreshing — showing no games rather than stale data.
            </div>
          )}
      ...
      {/* ── AGENTS — automated workers powering Vegas Vault AI ──
          Real status now lives in the AgentsPanel component, which polls
          /api/agents/status (heartbeats the crons write when they run). ── */}
      {authUser && isSubscribed && shellView === 'agents' && <AgentsPanel />}
                      ...
                      <div style={{ background:'rgba(255,255,255,0.7)',border:'1px solid rgba(255,255,255,0.93)',borderRadius:14,padding:'14px 16px',marginBottom:12 }}>
                        ...
                        <div className="vv-modal-2col" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10 }}>
                          <div style={{ padding:'10px 12px',background:'rgba(246,249,246,0.7)',border:'1px solid rgba(195,240,195,0.5)',borderRadius:10 }}>
                            ...
                            {game.awayPitcherStats && <div style={{ fontSize:9,color:'#aaa',marginTop:5,lineHeight:1.5,borderTop:'1px solid rgba(0,0,0,0.05)',paddingTop:5 }}>{new Date().getFullYear()} season: {game.awayPitcherStats}</div>}
                          </div>
                          <div style={{ padding:'10px 12px',background:'rgba(246,249,246,0.7)',border:'1px solid rgba(195,240,195,0.5)',borderRadius:10 }}>
                            ...
                            {/* Secondary: current season stats for context */}
                            {game.homePitcherStats && <div style={{ fontSize:9,color:'#aaa',marginTop:5,lineHeight:1.5,borderTop:'1px solid rgba(0,0,0,0.05)',paddingTop:5 }}>{new Date().getFullYear()} season: {game.homePitcherStats}</div>}
                          </div>
                        </div>
